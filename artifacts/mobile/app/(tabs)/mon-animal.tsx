...
export default function MonAnimalScreen() {
  ...
  const primaryAnimal = getAnimalById(quizResult.primary);
  const secondaryAnimal = getAnimalById(quizResult.secondary);

  const locale = lang === 'en' ? 'en-GB' : lang === 'fr' ? 'fr-FR' : 'fr-FR';
      ...
      <View style={{ padding: 20, gap: 18 }}>
        ...
        <View style={styles.row}>
          <View style={[styles.halfCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={[styles.animalBadge, { backgroundColor: primaryTotem.couleur + '30', borderColor: primaryTotem.couleur + '60' }]}>
              <Text style={[styles.animalBadgeText, { color: '#FFFFFF' }]}>{primaryTotem.nom}</Text>
            </View>
          ...
          <View style={[styles.halfCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={[styles.animalBadge, { backgroundColor: secondaryTotem.couleur + '30', borderColor: secondaryTotem.couleur + '60' }]}>
              <Text style={[styles.animalBadgeText, { color: '#FFFFFF' }]}>{secondaryTotem.nom}</Text>
            </View>
        ...
        {primaryAnimal && (
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <Text style={[styles.cardText, { color: colors.ivory }]}>{primaryAnimal.enseignementDuJour}</Text>
          </View>
        )}
        ...
        <View style={{ gap: 12 }}>
          ...
          {primaryAnimal && (
            <Pressable
              style={({ pressed }) => [{ opacity: pressed ? 0.88 : 1 }]}
              onPress={() => router.push(`/animal/${primaryAnimal.id}` as any)}
            >
              <LinearGradient colors={[colors.gold, colors.ochre]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.ctaBtn}>
...
const styles = StyleSheet.create({
  ...
  animalBadge: { borderRadius: 10, padding: 10, alignItems: 'center', borderWidth: 1 },
  animalBadgeText: { fontSize: 16, fontWeight: '700' as const },
