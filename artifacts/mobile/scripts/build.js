...
async function downloadFile(url, outputPath) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Metro bundle error (HTTP ${response.status}):\n${body.slice(0, 5000)}`);
...
async function checkExpoPackageVersions() {
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      if (code !== 0) {
        reject(new Error(
          "Incompatible Expo package versions detected. Fix them before deploying:\n" + output
        ));
      } else {
        console.log("All Expo packages are compatible.");
        resolve();
      }
    });
  });
