...
function readFixture(name) {
  const fixture = fs.readFileSync(path.join(fixturesDir, name));
  return name.endsWith(".base64")
    ? Buffer.from(fixture.toString("ascii").replace(/\s+/g, ""), "base64")
    : fixture;
}
...
test("real-world fixtures return their encoded dimensions", () => {
  const progressiveJpeg = readFixture("progressive.jpg.base64");
  const offsetTiff = readFixture("offset-values.tiff.base64");
  const svgWithUnits = readFixture("units.svg");

  assert.equal(
    progressiveJpeg.includes(Buffer.from([0xff, 0xc2])),
    true,
    "JPEG fixture should contain a progressive SOF2 marker",
  );
  assert.deepEqual(imageSize(progressiveJpeg), {
    width: 3,
    height: 2,
    type: "jpg",
  });

  assert.equal(offsetTiff.readUInt32LE(18), 38);
  assert.equal(offsetTiff.readUInt32LE(30), 46);
  assert.deepEqual(imageSize(offsetTiff), {
    width: 733,
    height: 491,
    type: "tiff",
  });

  assert.equal(svgWithUnits.includes('width="320px"'), true);
  assert.equal(svgWithUnits.includes('height="180px"'), true);
  assert.deepEqual(imageSize(svgWithUnits), {
    width: 320,
    height: 180,
    type: "svg",
  });
